...
| MariaDB | §1 mirror | dockerproxy.net 主体（chart 主 image `mariadb:11.4.10` 走 docker.io；idc-k8s 节点上 `mariadb:11.4.10` 已 cache，cold pull 经 dockerproxy.net 实测稳） | 2 件 ACR 直拉：runner toolchain `apecloud-registry.cn-zhangjiakou.cr.aliyuncs.com/apecloud/e2e-ginkgo:main`（runner 主容器，bash + kubectl + tar + sha256sum，2026-05-05 idc-k8s host runner Job 实测可拉）+ `apecloud/kubeblocks-tools:1.0.0`（initContainer 拷 kubectl + musl loader 到 shared volume）；syncer image `apecloud/syncer`（mariadb-aware build，PR-built 待 PR #140 split-brain fence merge 后转 stable tag） | chart 0 ghcr.io 主体引用（已 audit `templates/cmpd*.yaml` + `values.yaml`：仅 docker.io mariadb 主体 + ACR syncer/runner）；间接通过 vcluster 内 chaos-mesh 自装时触发 → 走 §1 决策树第 3 行（ghcr.io → sideload pipeline） | chart 主体无必走 sideload 大 image（`mariadb:11.4.10` 约 600MB 走 docker.io mirror 已稳）；chaos-mesh ghcr.io image 后续走 §3 sideload | syncer mariadb-aware build 当前为 dev/PR image（PR #140 fence 等 reviewer，merge 后转 stable）；其他 chart-side 0 dev/PR-built image | Helen / Jack — alpha.16 chart 1.1.1-alpha.16 fresh-matrix 8/8 PASS（standalone × 2 + semisync × 2 + async × 2 + galera × 2，2026-05-04 22:04:03 idc vcluster k3d evidence pack sha256 `ae8bb7f7e980b02b426603fb95056c6f3485a83d6b0b82440639b8b8b11c160d`）；2026-05-05 02:32 host runner Job 在 idc-k8s `mariadb-runner` ns 内独立跑 1 cycle 8/8 PASS evidence pack sha256 `a7f7af208d4e778bb4c4c8dd702f4d01b244b16a1336d88163fbabc875265197`，runner 完全脱 Mac，vcluster API 走 NodePort `https://172.16.3.11:31090` 不走 port-forward。host-runner Job pattern 沉淀工作 in-flight（task #425） |
...
| Oracle | §3 ACR | n/a（chart 无 docker.io 主流依赖；busybox/alpine init 极少数情况走 §1 dockerproxy.net mirror） | 6-image 全 ACR cn-zhangjiakou stable tag：`apecloud-registry.cn-zhangjiakou.cr.aliyuncs.com/apecloud/oracle:12.2.0.1-ee`（enterprise DB image，几 GB）+ `apecloud/oracle-dg-helper`（DataGuard helper）+ `apecloud/oracle-init`（init container）+ `apecloud/observability-exporter:2.2.0` + runner toolchain `apecloud/kubeblocks-tools:1.0.0` + `apecloud/e2e-ginkgo:main` | n/a 直接（chart 0 ghcr.io 引用，已 audit）；间接通过 vcluster bootstrap infra 触发：chaos-mesh（vcluster 内自装时走 §1 决策树第 3 行 ghcr.io → sideload）+ vcluster syncer + cert-manager | Oracle 主 DB image `oracle:12.2.0.1-ee` 几 GB（Phase 2 PoC 走 §3 sideload pipeline，需 chart values override `imagePullPolicy: Never`）；KB dev/main image 同样 fallback sideload | n/a 当前（CDB tag 锁定 12.2.0.1-ee，0 dev/PR-built chart-side image；KB dev/main 不计入 chart 主体） | James / John — chart 0 ghcr.io / 0 docker.io 主流依赖（已 audit）；idc2 ACR probe 待 Phase 2 Gate A 实测（参考 Tom SQL Server 同 ACR endpoint `busybox spider` 2026-05-04 PASS 作为交叉基线）|
| OceanBase | §3 ACR 直拉（chart 全部 5 件，无 sideload）| n/a（chart 0 docker.io 依赖；vcluster 注入的 alpine:3.13.1 initContainer 走 docker.io mirror patch 绕过，不属 OB chart 主体） | 5-image 全 ACR cn-zhangjiakou stable tag：`apecloud/oceanbase-ent:4.2.1.7-107020012024061213-ocp`（主 DB image，约 3GB，ACR 直拉稳定）+ `apecloud/obagent:4.2.2-100000042024011120`（metrics）+ `apecloud/kubeblocks-tools:1.0.2`（kbagent / config-manager / kbagent-worker initContainer）+ `apecloud/syncer:0.7.2`（init-syncer，HA 逻辑）+ `apecloud/obtools:4.2.1`（kb-tools initContainer）；5 件全走 ACR 直拉，无 ghcr.io / docker.io 主体依赖 | vcluster 内 chaos-mesh Phase 2 自装时触发（走 §1 决策树第 3 行 ghcr.io → sideload pipeline）；vcluster syncer `loft-sh/vcluster:0.19.7` 走 `ghcr.nju.edu.cn` mirror via helm upgrade，不属 OB chart 主体 | OB chart 主体无 sideload 需求（5 件全 ACR）；vcluster bootstrap 阶段 ghcr.io syncer image 用 `ghcr.nju.edu.cn` mirror 实测稳（2026-05-04）；chaos-mesh sideload 待 Phase 2 实操 | n/a（stable tag 锁定，0 dev/PR-built；syncer:0.7.2 含 PR #139 peer-primary guard fix） | Noah / Mia — idc4 vcluster `ob-vcluster`（ob-vcluster ns，host k8s 3 nodes）：Phase 1 smoke PASS 7/0/0 ✅ / Phase 2 chaos PASS（C03 FAIL_INTENDED + C04 + C07）✅ / Phase 3 chaos PASS（C05 C06 C10）✅ / 24h soak R2 running 2026-05-05 |
...
| idc4 | ⚠️ OB chart 主体 0 docker.io 依赖；vcluster 注入 `alpine:3.13.1` initContainer 走 `m.daocloud.io/docker.io` mirror patch（autopatcher daemon，2026-05-04 实测稳） | ✅ 5-image OB enterprise chart 全 ACR cn-zhangjiakou 直拉成功（idc4 节点 3 台：`192.168.10.44/45/46`，containerd `v1.7.15`/`v1.7.28`，KB v1.0.2 已运行；2026-05-04 Phase 1 vcluster bootstrap + replication cluster smoke PASS 实测，无 credentials blocker） | ❌ 不可达（与 idc2 一致）；vcluster syncer `loft-sh/vcluster:0.19.7` 走 `ghcr.nju.edu.cn` mirror via helm upgrade 绕过（2026-05-04 实测）；chaos-mesh Phase 2 走 §3 sideload | ⚠️ 部分：vcluster syncer 走 mirror（非 sideload）；debian:12-slim helper pod pattern 已知（与 idc/idc2 同 family）；OB chart 主体 0 必走 sideload |
...
- §5 各 line 行：Helen / Tom / James / Ben&Ava / Noah&Mia OB 已 fill ✅；所有 line 已收口
- §6 idc4 行（Noah / Mia）— 已 fill：ACR 直拉 ✅ / dockerproxy.net mirror patch ⚠️ / ghcr.io ❌ mirror 绕过 / sideload pipeline ⚠️ 部分；chaos-mesh sideload Phase 2 实操待补
