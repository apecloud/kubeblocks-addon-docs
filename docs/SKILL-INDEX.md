...
- [`addon-evidence-discipline-guide.md`](addon-evidence-discipline-guide.md) — 对自己产出的结论也做 bounded retry：N=1→"average" / 间接旁证→"系统性证伪" / 动机假设→narrative inflation 三类反模式 + harness 修复时机 dirty→clean 三段式 timeline（Phase 1 pre-sample / Phase 2 pre-promote reject-and-rerun / Phase 3 post-promote invalidate-and-re-promote）
...
- [`docs/addon-idc-image-registry-mirror-guide.md`](addon-idc-image-registry-mirror-guide.md) — IDC vcluster 测试环境的镜像供给三档决策树（mirror 主路径 / ACR 直拉 / sideload 兜底）+ chart audit checklist（避免 ghcr.io / docker.io 主依赖）+ vcluster 内 chaos-mesh 的 host k8s ↔ vcluster 双层 syncer 注入路径；含 6 line × 3 IDC（idc / idc2 / idc4）行的 ground-truth cross-engine reuse 表（Valkey / MariaDB / SQL Server / Oracle / OceanBase / KBE 各自的 image inventory + 路径选择 + 实测 evidence）；§7 syncer auto-sync open question 待 Phase 2 PoC 验证
