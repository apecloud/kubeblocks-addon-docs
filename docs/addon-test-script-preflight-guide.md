...
| **MySQL** | adoption + fingerprint evidence | MySQL line 从 idc 接入起即采用独立 kubeconfig + purpose-tagged file 模式（`~/.kube/config-idc2` + `mysql-test-host` / `mysql-test-pf`），实证 same-family preventive discipline；提供 KubeBlocks CRD count = 28 invariant 实证 + KB image multi-source via `ParametersDefinition.toolsSetup.toolConfigs[].image` 实证 | runner 启动不验 fingerprint → addon install/upgrade 路径 silent 落到错 cluster | 同 SQL Server 三层防御模式（preventive adoption） |
...
> **Why these four**: 我们观察到 cross-line incident 集中在四类 trigger 上——destructive op 入口（context 漂）、ns 切换（误删跨 line 资源）、multi-cluster co-host（cluster fingerprint 漂）、network topology transition（client-side proxy 拦截 / TLS SAN 不对）。每一类都有 cross-line incident 或 supporting datapoint 支撑（不是凭空设计），具体证据强度见 §0.5。
...
- Evidence partner: William (MySQL line) — N=3+adoption evidence pool elevation；§2 row 3 falsification with `--context=<intended>`；KubeBlocks CRD count = 28 N=2 invariant；MySQL line same-family preventive discipline (purpose-tagged kubeconfig 文件模式)
