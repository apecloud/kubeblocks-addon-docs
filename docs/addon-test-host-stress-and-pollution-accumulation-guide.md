...
**关键证据**：移除压力源（清掉测试 namespace 里的 polluted resources）后，**通常不需要 docker restart 也不需要 k3d 重建**，控制面会在 1-2 分钟内自愈：`/readyz` 回 ok、CSI plugin 停止 CrashLoop、k3d CPU 显著回落。这一现象**跨引擎双 N=2 独立验证**：

| 时点 | 引擎 | cleanup 前 | cleanup 后 | 是否 hot-fix |
|---|---|---|---|---|
| 04-29 00:41 | Valkey | CSI 3/4 CrashLoop, /readyz InternalError, CPU 1645% | CSI 4/4 Running, /readyz ok, CPU 210% | 未触发 |
| 04-29 22:07 | MariaDB | CSI 3/4 Error CrashLoop, /readyz 5×fail (etcd readiness 抖) | CSI 4/4 Running, /readyz 5×ok sustained 90s, RESTARTS frozen | 未触发 |

跨引擎重复观察的强信号，是本文档最核心的产物。
...
|---|---|---|
| **cell-1**：dual-engine + 双污染 | ✅ 红线 | MariaDB #396 R2/R2-replacement (04-28)：CSI 3/4 Error + sidecar storm，详见 [appendix C](#appendix-c-cell-1-mariadb-pollution) |
| **cell-2**：single-engine 多 suite + 污染累积 | ✅ 红线 | Valkey #208 Round 2 (04-28)：CSI 3/4 CrashLoop, /readyz InternalError, CPU 1645%；清完自愈 (1645%→210%) |
| **cell-2'**：single-engine **frozen cluster preservation 累积** (新发现，04-29 22:00) | ✅ 红线 | 5 个 mariadb preserved cluster (live) + KB controller reconcile = CSI 3/4 Error CrashLoop, /readyz 5×fail, RESTARTS 23→38。即"非 active testing 的 frozen evidence cluster 也是 active stress 源" |
| **cell-3**：single-engine 干净 single-load (Valkey) | ❌ 不撞 | Valkey Round 3/4/5 + Round 6 + G2 5-round Switchover stress 累计 **9 trigger 点 0 false-positive**：CSI RESTARTS 22→23 (+1) 全期，详见 [appendix B](#appendix-b-cell-3-valkey-solo) |
| **cell-4**：single-engine 干净 single-load (MariaDB) | ❌ 不撞 | MariaDB cell-4 R1 (`mdb-vc396r1-134638`，KB 1.0.2 + alpha.11)：T1-T11/CM1-4/R3 全过；T12 csi-resizer single restart drift 触发 hard-freeze 但 cascade shape absent (4/4 Running, /readyz 5×ok, CPU 256%)；详见 [appendix D](#appendix-d-cell-4-mariadb-solo) |

**从矩阵推导出的方法论**：

- cell-2 红线 + cell-3 绿 → 区分点是 **runner cleanup gate**：cleanup gate 不严会让 sequential 退化成"等效多 suite 同时占资源"
- cell-3 绿 → cascade 不是 single-engine workload 的固有属性，而是"压力如何累积"决定的
- cell-1 红 + cell-2 红 + cell-3 绿 + cell-4（待填）的组合会决定 **dual-load 假设强弱**
- 不论 cell-4 结果如何，cell-2 已独立证明：**污染累积单方就足以触发 cascade**，dual-load 不是必要诱因
...
### 第五步：preserve evidence ≠ preserve live cluster（重要 reframe）

**之前的反模式**：cell-2 cascade 后我们 freeze 现场 + "preserve evidence cluster 不主动 cleanup"，期望保留 live 状态供后续诊断对照。

**真实代价（04-29 22:00 实际撞过）**：累积 5 个 mariadb preserved cluster + KB controller / dataprotection 持续 reconcile = **active stress 源**，把 host 推过 cell-2 cascade threshold（CSI 3/4 Error / `/readyz` 5×fail）。**保留 live cluster 自身就在制造 cascade conditions**。

**新硬规则（westonnnn 04-29 22:21 directive）**：

1. **freeze evidence 落盘后（tar + sha256）立即 cleanup live cluster** — default 不保留
2. **任何时刻保留的 frozen cluster 数 ≤ 1**（仅在明确诊断需要 reproduce 的窗口期保留 1 个，超出立即清）
3. **持久化 evidence package 是 source of truth**，live cluster 不是 — yaml dump / describe / events 在 freeze 包里就够了
4. **多 frozen cluster 累积 = active stress 源**，不是 idle 状态

**例外场景（罕见才保留 live）**：
- 撞 stuck Terminating / phantom namespace 这类需要 live debug 才能 reproduce 的边界情况
- 某个特定 reconcile race 必须看 controller 实时反应才能 attribute
- 即便如此，也不超过 1 个保留 cluster，且明确 deadline（24h 内 cleanup）
...
（cell-4 跑完后此章节可补 N=2 完整对账：mariadb solo 是否单独触发 cascade，会决定 "hot-fix 价值更多来自清污染" 这条假设的强度）
...
## 附录 B：Valkey cell-3 single-load reference

> 本附录与 [附录 D](#appendix-d-cell-4-mariadb-solo) 共同构成 **two-engine N=2 confirmation**：两个不同引擎在干净 single-load 下都没观察到 cascade 形态，从反向锁定"dual-load + 污染累积是 cascade 的因变量"假设。
>
> **Applies to KB version**：KB controller 镜像 `apecloud/kubeblocks:r08a-10182-10185-only-20260428-113240`（PR #10182 + #10185 only，**不含** #10184），addon `kb-addon-valkey rev 57` / `valkey-0.1.1`。

### 试验设计与基线

- **single-load**：仅运行 valkey 一个 addon 的 `--test all` 全量 suite（smoke + chaos + tls + upgrade + sentinel + acl + custom-secret + rebuild + regression + switchover-regression + r05-nodeport + r06-loadbalancer + r07-maxmemory + r08a-reconfig-dynamic）
- **sequential 模式**：suite 之间串行执行，禁止并行
- **cleanup gate**：每个 suite 之间运行 cleanup gate 清 PV residue + 删 namespace + 重建 namespace（这是把 cell-2 的"残留累积 = 等效 dual"通道关掉的关键步骤）
- **storage**：`local-path`（PVC 扩容/backup 等需要时切 `csi-hostpath-resize`）
- **3 轮**：Round 3 / Round 4 / Round 5，跨轮 baseline 不变

### 3 轮汇总

| 维度 | Round 3 | Round 4 | Round 5 |
|---|---|---|---|
| Raw `PASS/FAIL/SKIP` | 739/5/6 | 739/5/6 | 737/6/6 |
| **产品 fail** | **0** | **0** | **0** |
| chaos `PASS/FAIL/SKIP` | 143/0/0 | 143/0/0 | **143/0/0** |
| regression `PASS/FAIL/SKIP` | 112/1/0 | 112/1/0 | 110/2/0 |
| 其它 9 个 suite | 全 0 fail | 全 0 fail | 全 0 fail |
| Runner exit | 1（非产品 fail 计入） | 1 | 1 |

3 轮一致：sequential `--test all` 在 baseline controller 上 **0 product fail**，chaos 三连绿，cleanup gate 全 suite 生效。

### 关键正向信号（与 cascade 形成对照）

| 信号 | 观察 |
|---|---|
| **CSI plugin `csi-hostpathplugin-0`** | 全程 4/4 Running，跨 3 轮 RESTARTS 22→23（**单次 +1**，由 `csi-external-health-monitor-controller` 自身的 health-monitor 周期触发，不是 hostpath flap） |
| **host `/readyz`** | 全程 ok，post-run 20/20 |
| **runner / OpsRequest** | 没有 timeout、没有 hang，每个 suite 在 budget 内收口 |
| **cleanup gate** | post-run 无 `valkey-*` namespace、无 `vlk-*` cluster/pod/Ops/backup 残留 |
| **chaos C04 / C08 / C10 / C12 watchpoint** | C04 all-6-pod kill non-conclusive harness fix 实战通过（#203）、C10 / C12 bounded eventually verify 在并发写下站住（#206）|

→ **没有 cascade 形态触发**：CSI 不撞 hostpath flap、`/readyz` 不丢失、apiserver 不进入反应过载窗口、runner 不冻结。

### Round 3/4 与 Round 5 唯一差异

Round 5 regression `R07 hot backup with master kill` 出现 **non-conclusive harness/timing/read signal**：scale-in former master 期间一次 transient read empty。Live focus 复核：current primary `valkey-1` `DBSIZE=100` + 抽样 key 存在；replica `valkey-3` 同样存在。**不是产品丢数**，分类为 regression 路径上的 verify harness gap（chaos verify 已经在 #206 加了 bounded eventually wrapper，regression verify 还没扩展到同一保护），单独建 follow-up task `#209 harden Valkey regression scale-in data verify retry` 跟踪，**不阻塞 baseline 锁定**。

### 与 cell-1 / cell-2 形态的反差

| 维度 | cell-2（附录 A：Valkey 多 suite + 残留累积） | cell-1（附录 C：MariaDB dual + 双污染） | **cell-3（本附录）** |
|---|---|---|---|
| stress 模型 | valkey 多 suite + 残留累积 | dual + 双污染（valkey 残留 + mariadb 活动） | **干净 valkey single-load + cleanup gate** |
| CSI plugin | hostpath flap、子容器 restart 风暴 | 4/4 Error、sidecars ContainerCreating | **4/4 Running，跨 3 轮 RESTARTS+1** |
| host `/readyz` | timeout / TLS handshake fail | 滞后 ok 后撞 cascade | **全程 ok** |
| runner | 冻结 | 冻结 | **正常收口** |
| 结论 | cascade ✅ | cascade ✅ | **no cascade ❌** |

### Evidence package

artifact root：
- Round 3：`/Users/wei/.slock/agents/54ed20d0-ac46-42e1-a88f-b54da489d065/artifacts/valkey-full-suite-baseline-round3-*`
- Round 4：`.../valkey-full-suite-baseline-round4-*`
- Round 5：`/Users/wei/.slock/agents/54ed20d0-ac46-42e1-a88f-b54da489d065/artifacts/valkey-full-suite-baseline-round5-080334`
  - `final/review-summary.md`（unified summary）
  - `final/summary.tsv`（per-suite table）
  - `focus/regression-r07-data-gap-093542/summary.txt`（R07 transient read live 复核）

附 baseline 锁定的 controller image identity：
- `apecloud/kubeblocks:r08a-10182-10185-only-20260428-113240`
- `imageID: sha256:f9bc6bb380f7b18f47944a750f2040a0d93f15b7c1e1ddfd8acbb65eaa840e74`

### 与 cell-4 的关系

cell-3（valkey）与 cell-4（mariadb，[附录 D](#appendix-d-cell-4-mariadb-solo)）一起完成 **two-engine N=2 confirmation**：

- 两个不同引擎、不同测试 runner、不同 chart、不同 storage class 缺陷模式，都在 single-load + cleanup gate 下未触发 cascade
- 跨引擎一致性排除 "cascade 是某个引擎特有 quirk" 的解释
- 与 cell-1 / cell-2 cascade 形态成镜像对照，把 stress 模型的因变量精准定位到 **dual-load + 污染累积**，而不是引擎、hot-fix、或某个 magic 变量

详细 reconciliation 在 chapter 7 用三段 framing 给出：cascade 形态 cross-engine 一致 → 不是引擎 quirk；no-cascade 形态 cross-engine 一致 → 不是 hot-fix magic；"hot-fix 修好了 cascade" 是 confounded action，真因变量是 stress 模型本身。

<a name="appendix-c-cell-1-mariadb-pollution"></a>
## 附录 C：MariaDB cell-1 case material

> **验证环境**：KB 1.0.2 + chart `mariadb-1.1.1-alpha.11`（含 12 tests-only patches）。Host: Wei's Mac (Jason 管)。Sample 时间窗：04-28 18:35-19:41。
...
## 附录 D：MariaDB cell-4 single-load reference

> **验证环境**：KB 1.0.2 + chart `mariadb-1.1.1-alpha.11`（含 12 tests-only patches）。Host: Wei's Mac (Jason 管)。Sample 时间窗：04-29 13:46-14:14。
> **two-engine N=2 confirmation**：本附录与 [appendix B](#appendix-b-cell-3-valkey-solo) 一起证 "干净 single-load 不论引擎都不撞 cascade"。

### 起跑前置

mariadb cell-4 R1 起跑前 host preflight (Jason `preflight-mariadb-solo-20260429T032932Z.txt`)：CSI 4/4 Running RESTARTS=23 frozen，sidecars 1/1，`/readyz` ok。
...
| chaos C1 / C2 / C3 | ✅ PASS |
| chaos C4 rapid primary kills round-1 | ❌ FAIL — observation-class（pod_count=2, primary=1, secondary=none, role_holes=22, route_holes=0, read_holes=0 → 数据健康但 role label 未在 bounded window 内 re-publish）|
| chaos C5 / C6 / C8 | ❌ FAIL — observation-class |
| T12 VolumeExpansion | ❌ FAIL — observation-class no-secondary，**同时**触发 `csi-hostpath-resizer-0` RESTARTS 0→1 → hard-freeze runner |
| C7 / T13 / T14 | SKIP — capability-aware |

### cell-4 question 答案：no host cascade ✅

freeze 时点 host 状态对照 cell-2 cascade 形态：

| 信号 | cell-2 cascade | cell-4 freeze 时刻 |
|---|---|---|
| `csi-hostpathplugin-0` | 3/4 CrashLoopBackOff | **4/4 Running**, RESTARTS=23 frozen |
| sidecars | ContainerCreating 风暴 | 1/1 Running，仅 resizer +1 单点 restart |
| `/readyz` | InternalError + etcd-readiness fail | **5×ok** |
| host k3d CPU peak | **1645%** | **256.75%** |

→ T12 csi-resizer 单一 restart 是 VolumeExpansion 期间 sidecar reconnect class，**不构成 cascade shape**。cell-4 cascade question 答案: NO cascade ✅

### Route B Attribution data (R1 PASS path 5-segment timing)

为了 attribute observation gap 的 latency，跑了 instrumented run（dev1 chart + temp overlay）。Sample `mdb-rolepub-161841` C4 round-3 timing decomposition（PASS 路径 ~43s total）：

| Segment | Latency | 占比 |
|---|---:|---:|
| kill → KB API ack (new pod uid) | 1.137s | 3% |
| **uid → ready (DB restart + replication)** | **21.762s** | **51%** |
| ready → probe secondary (probe interval) | 8.630s | 20% |
| probe → kbagent push event | 0.747s | 2% |
| event → KB controller label publish | 0.695s | 1.6% |
| **label → runner observe pacing** | **10.449s** | **24%** |
| **total** | **43.968s** | |

**关键 finding**：**KB controller label/event 合计仅 ~3%**，**disprove "controller-bottleneck" 假设**。Attribution 主导段是 DB restart (51%) + roleProbe cadence (20%) + runner observe pacing (24%)。

### Multi-phase observation gap evidence

cell-4 R1 的 chaos C4-C8 + T12 fails 之外，其他 instrumented runs 也撞 observation gap，**跨多 phase**（不只 chaos kill 触发）：

| Sample | Phase first fail | Condition |
|---|---|---|
| `mdb-rolepub-164018` | T8 Restart + T9 Switchover | single-load supplementary |
| `mdb-rolepub-204819` | T7 VScale | single-load + CSI 23→25 drift |
| `mdb-rolepub-221722` | CM3 sync_binlog reconfigure | dual-load (with valkey G2) |
| `mdb-rolepub-222841` | T3 service VIP + CM2/CM3/CM4 reconfigure Failed | dual-load |

**累计 phase 范围**：T3 / CM2-CM4 / T7 / T8 / T9 / C4-C8 / T12 — 至少 7+ phase 都撞同 observation-class fail（数据健康但 role label / OpsRequest 状态滞后）。**推翻** "只由 C4 chaos kill 特殊触发" framing。

### 跨引擎 N=2 confirmation
...
R1 attribution evidence:
- tar: `artifacts/mdb-rolepub-161841-task396-rolepub-debug-161841.tar.gz`
- sha256: `fa1e9440e465f1cbdb13f6f21d6c5d24de6b41c50d16b7297a10e70a7ed5ce8f`

cell-4 R1 freeze evidence (T12 csi-resizer hard-freeze):
- tar: `artifacts/mdb-vc396r1-134638-task396-cell4-csi-resizer-freeze-141532.tar.gz`
- sha256: `3085a16456b0812fe94be6e0e25fbc682ad323bb08de85c76701a47cc06f999c`

Multi-phase fail samples 各有独立 evidence packages（详见 task #396 closure report）。

### Three actionable follow-ups (拆 task)

1. **roleProbe cadence/latency 优化**：5s 间隔太松（贡献 20% latency），可缩到 2-3s 或加事件驱动 trigger（pod 状态变化立即跑一次）
2. **runner deadline 分层口径**：120s 一刀切不合理，per-phase deadline（VScale / Restart / chaos / reconfigure 各自 budget），按 P95+margin 标定
3. **helper observability 补强**：per-tick DB truth + stderr/artifact 持久化，让 fail-path 5-segment timing future runs 也能 capture（不依赖 instrumentation overlay）
