...
下表来自 MariaDB addon 的实战，全部归同一类 root cause；每行末尾标该反模式对应的硬规则（见后文 7 条规则）：

| 现象 | 错误判定方式 | 真实状态 | 修复 | 对应规则 |
|---|---|---|---|---|
| CM4 primary-switch convergence helper 报 `reconverged after 61s deadline=60s` | awk 用 `last_elapsed`（最后一个采样的相对时间）作为 reconverged 时间 | 实际 25-49s 就 1P1S 稳定，但 watch 跑满 60s | 跟踪真实 `post_gap_reconverged_ts`，用它做 deadline 检查 | Rule 1 + Rule 2 + Rule 7 |
| `assert_replication_ok` 报 `Slave_SQL_Running=''` | bounded loop 等 IO=Yes，SQL 是 loop 外**一次性**读 | rolling restart 后 IO 先来、SQL 后到，几秒内都会到 Yes/Yes | bounded loop 等 **IO 和 SQL 都 Yes** | Rule 1 + Rule 4 |
| `assert_data_consistent` 报 `no primary` | `primary_idx` 是单次读 | role label 短暂 flicker（pod 重建期间），几秒后稳定 | bounded retry 给 `primary_idx`/`secondary_idx` 30s | Rule 1 + Rule 4 |
| SQL 探针报 `classification=route_api` | helper 已自愈了 route，但探针不重试 | route 重建后已可用 | bounded retry on `route_api` 分类（其他分类不重试，避免掩盖真问题） | Rule 1 + Rule 3 + Rule 5 |
| addon `finalize_replication_rejoin_ready_gate` 让 pod 卡 `.replication-pending` | START SLAVE 后**立刻**判定 slave_status，IO 还在 Connecting | 几秒内 Slave_IO/SQL 都会 Yes | bounded loop 30s 内 re-query slave_status，收敛就 mark_replication_ready | Rule 1 + Rule 8 (addon-side) |

每一个看起来像独立 bug，但全是同一类 — **对异步系统做同步式 snapshot 判定**。

**Cross-engine scope caveat**：以上 5 条都来自 MariaDB addon 实战。其他引擎（valkey / oracle / postgresql 等）的具体反模式可能在收敛 step 数、超时数值上不同，但 root cause 同类（"对异步系统做单次 snapshot"）。timeout 数值（30s / 60-90s）跨引擎需要按各自 P95 重新校准。
...
3. **观测面 fail 与产品 fail 必须分类**。bounded retry **只对 transient classifications** retry（`route_api`、`empty_output`、role-flicker、Connecting 等）。**不要**对真实 `*_mismatch` retry — 那等于把真 bug 盖掉。
   - **应用场景**：决定哪类 probe failure 该 retry vs fail-fast
4. **状态来源要每次 refresh**。loop 里如果只用第一次的 snapshot，bounded retry 等于死循环 + sleep。
   - **应用场景**：写 bounded retry loop body
5. **early break on definitively broken**。例如 GTID divergence、real_data_mismatch 等已确认坏态，立刻退出别等 timeout。
   - **应用场景**：写 bounded retry loop body — 区分"still converging"vs"won't converge"
6. **timeout 选 P95 + safety margin**，不是平均值。常见 IO+SQL 收敛 5-15s，给 30s；rolling restart 角色收敛 30-50s，给 60-90s。给得太紧（边界 ±1s）会变成噪音 bug，给得太松（10×P95）会掩盖真慢化。这些数字是 MariaDB P95；其他引擎要按各自 P95 重新校准。
   - **应用场景**：选 timeout 参数时
7. **真实事件时间戳 vs 采样时间戳要分开**。helper 报"reconverged at X"时，X 必须是事件首次发生的时间，不是 watch 文件最后一行的时间。命名混淆是反复出现的源 bug。
   - **应用场景**：报"X happened at time T"时（T 必须是事件时间不是采样时间）
8. **addon 业务代码也踩同一类反模式**。前 7 条不只适用测试 helper —— addon 内部 bootstrap / rejoin / Probe / Action 任何"动作完成立即判定后果"的代码都同样要 bounded retry。MariaDB `finalize_replication_rejoin_ready_gate` 反模式就是 addon-side example。
   - **应用场景**：写 addon 业务代码（启动 / rejoin / reconfigure / Probe / Action 后判定）时
