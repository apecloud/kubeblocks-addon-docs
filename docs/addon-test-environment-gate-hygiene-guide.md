...
| 场景 | 关键 gate |
|---|---|
| 跑任何 smoke / chaos / regression 之前 | 7 项全跑一遍 |
| 测试机重启 / Docker 重启 / k3d 集群被动恢复之后 | 7 项全跑（**禁止复用 pre-restart 任何事实**） |
| T1 创建集群阶段 timeout 但拿不到 DB 层证据 | 倒查 layer 1（路由）+ layer 3（CSI） |
| PVC 永远 Pending、Pod 永远 Pending、ImagePullBackOff | 倒查 layer 3（CSI）+ layer 4（镜像分发） |
| 设计新 capability-aware skip | 显式记录 capability flag，区分"环境真不支持"vs"环境暂时不可读" |
| 判 first blocker 时分类 | 分清"产品 fail"vs"环境 fail"vs"测试 fail" 三层 |
