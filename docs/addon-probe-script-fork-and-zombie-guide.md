...
## 案例附录：MariaDB roleProbe `printf | grep -q` Pattern B 实证

MariaDB addon `1.1.1-alpha.12`（roleProbe cadence 5s→3s 后第一版），04-30 task #402 给 idle cluster 做长时间 zombie 验证。预期按本文 audit checklist negative case（`mariadb-fork-audit-pass-negative-case.md`）所述 4h 全 0 zombie。

实测：

| 采样点 | total zombies | 分布 |
|---|---|---|
...
2h redline 触发 freeze + reverse investigation，机制坐实为 **Pattern B — 隐式 timeout-kill orphan**：

- `replication-roleprobe.sh` secondary 路径 4 个 `printf | grep -q` pipeline
- `roleProbe.timeoutSeconds=1` 偶发被 SHOW SLAVE STATUS 越界 → kbagent SIGKILL 脚本主进程 → grep 子进程 reparent 到 kbagent (PID 1, Go 不 reap) → zombie

**Leak rate ≈ 1/2400 probes ≈ 0.04%**。远低于 Pattern A 的 valkey check-role.sh `5-14/min` storm，但**机制同源**。

按 4D audit checklist：
...
| reaper (Z) | non-reaper — kbagent (Go) 不 reap |
...
完整时间线 + zombie scan 证据 + fix 路径见 [`cases/mariadb/mariadb-roleprobe-timeout-kill-orphan-case.md`](cases/mariadb/mariadb-roleprobe-timeout-kill-orphan-case.md)。

## 案例附录：MariaDB fork audit pass — Galera + cmpd-replication entry pattern

Valkey check-role.sh fork 事件触发 cross-engine fork pattern audit 后，MariaDB addon **audit pass，但不是因为没有 fork pattern**：

- `addons/mariadb/scripts/galera-start.sh:89` 后台 polling `wsrep_local_state` 直到 SYNCED
- `addons/mariadb/templates/cmpd-replication.yaml:639` 后台启 mariadbd（标准 entrypoint pattern: bash 先后台启 mariadbd 再做 user/replication setup）
- `addons/mariadb/templates/cmpd-semisync.yaml:114` 同上
- `addons/mariadb/templates/cmpd-semisync.yaml:280` 后台 monitoring loop 检查 semi-sync deadlock

四处都落 4D audit checklist 的 **(container main, A, low, non-reaper) → ✅ acceptable / out-of-scope** cell — **有 fork、低频、单次容器寿命内 1 次、不堆积**。

这条 case 的价值：

1. 给后续 reviewer 一个具体例子，**看到 fork 不要直接 block**，按 4D 表 attribute
2. 给 MariaDB addon 自己一份长期记录，**禁止后续把这些 fork pattern 改成高频**（会跨格成 leak）
3. evidence-discipline 反例：audit 一开始 over-broad（"找 fork = 找 bug"）的反模式

完整 audit 命令 + 4 处 entry attribute 表 + cross-engine 复用建议见 [`cases/mariadb/mariadb-fork-audit-pass-negative-case.md`](cases/mariadb/mariadb-fork-audit-pass-negative-case.md)。

## 案例附录：OceanBase audit pass scope carve-out

OceanBase addon 跑过 4D audit checklist 后**没有命中 Pattern A/B leak**，结论 out-of-scope：

- `bootstrap.sh:24` 14 处 lifecycle pipeline — context = container main (observer entrypoint)，PID 1 是 observer 但容器寿命内 fork **不进 audit**
- `cat /tmp/ready` readinessProbe — 走 kubelet-scheduled native probe path，没有 fork

OB 跑了 audit、确认本 addon 不踩 Pattern A/B → 写"out-of-scope" verdict 进 4D 表（本文 line 119-120），**没找到失败现场，也没产生 OB fix PR**。这是"用 doc 跑 audit 但结果是 negative"的合法终态：用 doc 不一定要每个引擎都修代码，**确认不需要修也是有用结果**。
