...
下面命令以 Valkey G7 cluster 为例（`NS=valkey-g7`，cluster `vlk-tls`，derived secret 名字按 KB 渲染规则 `<cluster>-<component>-conn-credential`），引擎切换时把 secret name 与 cli 客户端命令换掉即可。

#### 1. patch derived secret（方案 C 第一步）

```bash
NS=valkey-g7
DERIVED_SECRET=vlk-tls-valkey-conn-credential
NEW_CRT_B64=$(base64 -w0 < /tmp/new.crt)
NEW_KEY_B64=$(base64 -w0 < /tmp/new.key)
kubectl -n "$NS" patch secret "$DERIVED_SECRET" --type=json -p "$(cat <<JSON
[
...
#### 2. 在每个 pod 内验证挂载文件已更新（验证口径 #2）

```bash
for pod in $(kubectl -n "$NS" get pod -l app.kubernetes.io/instance=vlk-tls -o name); do
  echo "=== $pod ==="
  kubectl -n "$NS" exec "$pod" -- openssl x509 -noout -dates -in /etc/tls/tls.crt
done
```

输出里 `notAfter=` 必须等于新 cert 的 NotAfter；如果还是旧值，等 mount sync（一般 20-60s）然后再读一遍，仍不更新就检查 kubelet event。

#### 3. rolling restart pod 让业务进程重新加载（方案 C 第二步）

```bash
kubectl -n "$NS" rollout restart statefulset vlk-tls-valkey
kubectl -n "$NS" rollout status statefulset vlk-tls-valkey --timeout=300s
```

集群有 switchover capability 时优先用 OpsRequest restart 走 KB 顺序，避免业务读写抖动。

#### 4. 验证业务进程层 + 内部连接层（验证口径 #3 / #4）

```bash
# 口径 #3：业务进程层 — 用 TLS 客户端从外部建新连接握手成功
kubectl -n "$NS" exec vlk-tls-valkey-0 -- \
  valkey-cli --tls --cacert /etc/tls/ca.crt -h vlk-tls-valkey-0 PING
# 期望：PONG

# 口径 #4：内部连接层 — primary / replica 之间复制链路恢复
kubectl -n "$NS" exec vlk-tls-valkey-0 -- \
  valkey-cli --tls --cacert /etc/tls/ca.crt INFO replication \
  | grep -E 'role:|master_link_status:|connected_slaves:'
# 期望：role 一致 + master_link_status:up + connected_slaves > 0
```

任何一条命令失败都按对应验证口径定位（#3 / #4 任一不过就是 silent failure 的现场，必须重新走方案 C）。
